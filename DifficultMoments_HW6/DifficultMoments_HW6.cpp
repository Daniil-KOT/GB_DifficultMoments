...
int PrimeNumber(int ordinalNum)
{
	...
	{
		cout << "Progress: " << (static_cast<double>(counter) / ordinalNum) * 100 << "%";
		if (counter < 100)
		{
...
int main()
{
	...
	thread thStor(pcout<Storage>, storage);

	thread thStr(pcout<std::string>, "Hello world!");

	thread thInt(pcout<int>, 25);
	...
	std::cin.get();
	int result = 0;
	thread CalculatePrimeTh([&result]() { result = PrimeNumber(10); });
